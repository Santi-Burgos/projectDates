  ...
  public void createConcept(Context ctx) {
    try {
      ...
      if (scheduleJson != null && !scheduleJson.isEmpty()) {
        schedule = ctx.jsonMapper().fromJsonString(scheduleJson, new TypeReference<List<TimeRange>>() {
        }.getType());
      }
      ...
      ConceptResponse newConcept = conceptService.createConcept(conceptToCreate, imageContent, fileName);
